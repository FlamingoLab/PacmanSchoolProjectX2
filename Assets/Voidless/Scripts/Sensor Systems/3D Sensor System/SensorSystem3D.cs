	...
	public bool GetSubsystemDetection(int _subsystemIndex)
	{
		...
		foreach(SensorData3D sensorData in subsystem)
		{
			...
			switch(sensorData.sensorType)
			{
				...
				case SensorType.Box:
				return Physics.BoxCast(ray.origin, sensorData.dimensions, ray.direction, transform.rotation, sensorData.distance, subsystem.layerMask);

				case SensorType.Sphere:
				return Physics.SphereCast(ray, sensorData.radius, sensorData.distance, subsystem.layerMask);

				case SensorType.Capsule:
				Vector3 destinyPoint = ray.origin + (ray.direction * sensorData.distance);
				return Physics.CapsuleCast(ray.origin, destinyPoint, sensorData.radius, ray.direction, sensorData.distance, subsystem.layerMask);
			}
		}
	...
	public bool GetSubsystemDetection(int _subsystemIndex, out RaycastHit _hit)
	{
		...
		foreach(SensorData3D sensorData in subsystem)
		{
			...
			switch(sensorData.sensorType)
			{
				...
				case SensorType.Box:
				return Physics.BoxCast(ray.origin, sensorData.dimensions, ray.direction, out _hit, transform.rotation, sensorData.distance, subsystem.layerMask);

				case SensorType.Sphere:
				return Physics.SphereCast(ray, sensorData.radius, out _hit, sensorData.distance, subsystem.layerMask);

				case SensorType.Capsule:
				Vector3 destinyPoint = ray.origin + (ray.direction * sensorData.distance);
				return Physics.CapsuleCast(ray.origin, destinyPoint, sensorData.radius, ray.direction, out _hit, sensorData.distance, subsystem.layerMask);
			}
		}
	...
	public bool GetSubsystemDetections(int _subsystemIndex, out RaycastHit _hit)
	{
		...
		foreach(SensorData3D sensorData in subsystem)
		{
			...
			switch(sensorData.sensorType)
			{
				...
				case SensorType.Box:
				if(!Physics.BoxCast(ray.origin, sensorData.dimensions, ray.direction, out _hit, transform.rotation, sensorData.distance, subsystem.layerMask)) return false;
				else break;

				case SensorType.Sphere:
				if(!Physics.SphereCast(ray, sensorData.radius, out _hit, sensorData.distance, subsystem.layerMask)) return false;
				else break;

				case SensorType.Capsule:
				Vector3 destinyPoint = ray.origin + (ray.direction * sensorData.distance);
				if(!Physics.CapsuleCast(ray.origin, destinyPoint, sensorData.radius, ray.direction, out _hit, sensorData.distance, subsystem.layerMask)) return false;
				else break;
			}
		}
